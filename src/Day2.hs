...
divs
  ...
  -> a
  -> Maybe (a, a)
divs rows x = (,) x <$> divisor
  where divisor = find divides rows
        divides y = y /= x && y `rem` x == 0


rowFactor
  :: (Integral a, Functor t, Foldable t)
  => t a
  -> (a, a)
rowFactor row = fromJust . join $ find isJust $ divs row <$> row


divsChecksum :: Integral a => [a] -> Int -> a
divsChecksum sheet nrows = sum factors
  where
    factors = uncurry (flip div) <$> (rowFactor <$> rows)
    rows    = chunksOf nrows sheet

minMaxChecksum :: (Num a, Ord a) => [a] -> Int -> a
minMaxChecksum sheet nrows = sum diffs
  where diffs = zipWith (-) maxs mins
        maxs  = maximum <$> rows
        mins  = minimum <$> rows
        rows  = chunksOf nrows sheet

answer1 :: Int
answer1 = minMaxChecksum input 16

answer2 :: Int
answer2 = divsChecksum input 16

input :: [Int]
input = [4168,  3925,	858,	2203,	440,	185,	2886,	160,	1811,	4272,	4333,	2180,	174,	157,	361,	1555,
